...
public class RouterTest {
    ...
    public void testGettingTheCorrectResponseType() throws IOException {
        ...
        assertEquals("java.io.SequenceInputStream", router.routeRequest(mockRequest).getClass().getName());
        assertTrue(specHelper.responseString(router.routeRequest(mockRequest)).contains("200"));

        assertEquals("java.io.SequenceInputStream", router.routeRequest(mockRequest2).getClass().getName());
        assertTrue(specHelper.responseString(router.routeRequest(mockRequest2)).contains("200"));

        assertEquals("java.io.SequenceInputStream", router.routeRequest(mockRequest3).getClass().getName());
        assertTrue(specHelper.responseString(router.routeRequest(mockRequest3)).contains("404"));

        assertEquals("java.io.SequenceInputStream", router.routeRequest(mockRequest4).getClass().getName());
